...
AFRAME.registerComponent("orbit-controls", {
  ...
  update: function (oldData) {
    ...
    if (this.data.rotateTo) {
      var rotateToVec3 = new THREE.Vector3(
  ...
  tick: function (t) {
    var render =
      this.data.enabled && this.isRunning ? this.updateView() : false;
  ...
  onMouseDown: function (event) {
    ...
    if (
      event.button === this.mouseButtons.ORBIT &&
  ...
  pan: function (deltaX, deltaY) {
    ...
    if (this.cameraType === "PerspectiveCamera") {
      ...
      this.panHorizontally(
        (2 * deltaX * targetDistance) / canvas.clientHeight,
        this.object.matrix
      ); // we actually don't use screenWidth, since perspective camera is fixed to screen height
      this.panVertically(
        (2 * deltaY * targetDistance) / canvas.clientHeight,
        this.object.matrix
      );
    } else if (this.cameraType === "OrthographicCamera") {
      // orthographic
      this.panHorizontally(
        (deltaX * (this.dolly.right - this.dolly.left)) /
          this.camera.zoom /
          canvas.clientWidth,
        this.object.matrix
      );
      this.panVertically(
        (deltaY * (this.dolly.top - this.dolly.bottom)) /
          this.camera.zoom /
          canvas.clientHeight,
        this.object.matrix
      );
  ...
  dollyIn: function (dollyScale) {
    ...
    if (this.cameraType === "PerspectiveCamera") {
      ...
      this.camera.zoom = Math.max(
        this.data.minZoom,
        Math.min(this.data.maxZoom, this.camera.zoom * dollyScale)
      );
  ...
  updateView: function (forceUpdate) {
    if (this.desiredPosition && this.state === this.STATE.ROTATE_TO) {
      ...
      this.sphericalDelta.set(
        ...
        phiDiff * this.data.rotateToSpeed,
        thetaDiff * this.data.rotateToSpeed
      );
    }
    ...
    if (this.data.autoRotate && this.state === this.STATE.NONE)
